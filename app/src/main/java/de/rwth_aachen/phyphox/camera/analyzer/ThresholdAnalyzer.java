...
public class ThresholdAnalyzer extends AnalyzingModule {
    ...
    void drawThreshold(float[] camMatrix, RectF passepartout) {
        ...
        GLES20.glScissor((int)Math.floor(width*(1.0-Math.max(passepartout.top, passepartout.bottom))), (int)Math.floor(height*(1.0-Math.max(passepartout.left, passepartout.right))), (int)Math.ceil(width*Math.abs(passepartout.height())), (int)Math.ceil(height*Math.abs(passepartout.width())));
    ...
    void drawThresholdDownsampling(int step, float[] camMatrix) {
        ...
        GLES20.glUniform2f(thresholdDownsamplingResSourceHandle, step == 0 ? width : wDownsampleStep[step-1], step == 0 ? height : hDownsampleStep[step-1]);
