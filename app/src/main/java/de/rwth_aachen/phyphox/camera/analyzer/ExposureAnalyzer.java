...
public class ExposureAnalyzer extends AnalyzingModule {
    ...
    void drawExposure(float[] camMatrix, RectF passepartout) {
        makeCurrent(downsamplingFramebuffers[0], wDownsampleStep[0], hDownsampleStep[0]);
        ...
        GLES20.glUniform2f(exposureProgramResSourceHandle, width, height);
    ...
    void drawExposureDownsampling(int step, float[] camMatrix) {
        ...
        GLES20.glUniform2f(exposureDownsamplingResSourceHandle, step == 0 ? width : wDownsampleStep[step-1], step == 0 ? height : hDownsampleStep[step-1]);
